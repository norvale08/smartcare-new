...
const VitalPredictions: React.FC<VitalPredictionsProps> = ({ patient, vitals }) => {
  ...
  const getHistoricalRiskColor = (label: 'critical' | 'elevating' | 'stable') => {
    if (label === 'critical') return getRiskColor('high');
    if (label === 'elevating') return getRiskColor('medium');
  ...
  const historicalRisk = useMemo(() => {
    ...
    const structuredVitals = vitals
      .filter(reading => reading.systolic !== undefined && reading.diastolic !== undefined && reading.heartRate !== undefined)
      .map((reading) => ({
    ...
    <div className="bg-white rounded-lg border shadow-sm p-6">
      ...
      {historicalRisk && (
        <div className="border-t pt-6 mt-6">
          ...
          <div className={`p-4 rounded-lg border-2 ${getHistoricalRiskColor(historicalRisk.label)}`}>
            <div className="flex items-center justify-between mb-2">
              <span className="text-base font-semibold capitalize">{historicalRisk.label}</span>
              <span className="text-sm font-bold bg-white px-2 py-1 rounded-full border">
                Score {historicalRisk.score.toFixed(1)}
              </span>
            </div>
            <p className="text-sm text-gray-700 leading-relaxed">{historicalRisk.summary}</p>
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 mt-4 text-sm">
              <div className="p-3 rounded-lg bg-white/60 border border-gray-200">
                <div className="text-xs text-gray-500">Avg BP</div>
                <div className="font-semibold text-gray-900">
                  {Math.round(historicalRisk.averages.systolic)}/{Math.round(historicalRisk.averages.diastolic)} mmHg
                </div>
              </div>
              {historicalRisk.averages.heartRate ? (
                <div className="p-3 rounded-lg bg-white/60 border border-gray-200">
                  <div className="text-xs text-gray-500">Avg HR</div>
                  <div className="font-semibold text-gray-900">
                    {Math.round(historicalRisk.averages.heartRate)} bpm
                  </div>
                </div>
              ) : (
                <div className="p-3 rounded-lg bg-white/60 border border-gray-200">
                  <div className="text-xs text-gray-500">Readings used</div>
                  <div className="font-semibold text-gray-900">
                    {Math.min(vitals.length, 6)} recent
                  </div>
                </div>
              )}
              <div className="p-3 rounded-lg bg-white/60 border border-gray-200">
