...
interface VoiceControlPanelProps {
  ...
  onPauseResume?: () => void;
  // Kept for backwards compatibility (hypertension page can optionally pass this),
  // but this panel intentionally matches the simpler diabetes voice UX.
...
const VoiceControlPanel: React.FC<VoiceControlPanelProps> = ({
  ...
  const { active, listening, speaking, muted, paused, currentField } = voiceModeState;

  return (
    <div className="bg-emerald-500 rounded-lg p-2 sm:p-2.5 border border-white/20 mb-4">
        ...
        <div className="flex items-center gap-2 flex-1 min-w-0">
          ...
          <div className="flex-1 min-w-0">
            <p className="text-white font-semibold text-xs sm:text-sm truncate">
              ...
              {active && currentField ? (
                <span className="ml-2 text-white/80">• {currentField}</span>
              ) : null}
            </p>
          </div>
        </div>
